...
export const CartPage: React.FC = () => {
  ...
  const total = cartItems
    ...
    .reduce((total, item) => {
      ...
      console.log(`Item ${item.MaSP}: TongTien=${item.TongTien}, Parsed=${itemTotal}`);
  ...
  const handleRemoveSelected = async () => {
    ...
    if (!confirm(`Bạn có chắc muốn xóa ${selectedItems.length} sản phẩm?`)) return;
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-6xl mx-auto px-4">
          <div className="animate-pulse">
            {[1, 2, 3].map((item) => (
              <div key={item} className="bg-white rounded-lg shadow-sm p-6 mb-4">
