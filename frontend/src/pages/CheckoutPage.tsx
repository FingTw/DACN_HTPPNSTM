...
import type { ProcessCheckoutData, ShippingCalculationResult, OrderItem as BaseOrderItem } from '@/services/orderService';
...
const CheckoutPage: React.FC = () => {
  ...
  const [selectedShippingMethod, setSelectedShippingMethod] = useState<ShippingCalculationResult | null>(null);
    ...
    const loadAvailableVouchers = async () => {
      try {
        ...
        const validVouchers = response.data.filter(voucher => {
          ...
          const isUsageExceeded = hasUsageLimit && 
  ...
  const calculateDiscounts = useCallback((currentSubtotal: number, currentShippingFee: number, vouchers: AppliedVouchers) => {
    ...
    Object.entries(vouchers).forEach(([type, voucher]) => {
      ...
      if (km.DieuKien && km.DieuKien > currentSubtotal) {
        ...
        toast.error(`Voucher "${km.TenKM}" yêu cầu đơn tối thiểu ${km.DieuKien.toLocaleString('vi-VN')}đ`);
        return;
      }
  ...
  useEffect(() => {
    const result = calculateDiscounts(subtotal, dynamicShippingFee, selectedVouchers);
  ...
  const handleAddressSelect = useCallback((selectedAddress: Address) => {
    ...
    localStorage.setItem('savedShippingAddress', JSON.stringify(selectedAddress));
  ...
  const handleShippingSelect = useCallback((shipping: ShippingCalculationResult) => {
  ...
  const toggleVoucher = (voucher: KhuyenMaiDaNhan) => {
    ...
    const isShippingVoucher = km.LoaiKM === 'SHIPPING' || km.LoaiKM === 'FREESHIP';
    const voucherType = isShippingVoucher ? 'shippingVoucher' : 'discountVoucher';
  ...
  const getDiscountText = (voucher: KhuyenMaiDaNhan) => {
    ...
    if (km.HinhThucGiam === 'PERCENTAGE') {
      return `Giảm ${km.GiaTriGiam}%${km.SoTienGiamToiDa ? ` (tối đa ${km.SoTienGiamToiDa.toLocaleString('vi-VN')}đ)` : ''}`;
    } else {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const checkoutPayload: ProcessCheckoutData & { appliedVouchers?: string[] } = {
      ...
      console.log('💰 Final Payload với khuyến mãi:', checkoutPayload);
              ...
              {vouchersLoading ? (
                ...
                <div className="space-y-3">
                  ...
                  {availableVouchers.map(voucher => {
                    ...
                    const isShipping = km.LoaiKM === 'SHIPPING' || km.LoaiKM === 'FREESHIP';
                    const isSelected = isShipping ? 
                      selectedVouchers.shippingVoucher?.MaKM === voucher.MaKM :
                      selectedVouchers.discountVoucher?.MaKM === voucher.MaKM;
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <h3 className="font-semibold text-gray-900">{km.TenKM}</h3>
                            <p className="text-sm text-gray-600 mt-1">{km.MoTa}</p>
                            <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                              <span>{getDiscountText(voucher)}</span>
                              <span>•</span>
                              <span>HSD: {new Date(km.NgayKetThuc).toLocaleDateString('vi-VN')}</span>
                              {km.DieuKien > 0 && (
                                <>
                                  <span>•</span>
                                  <span>Đơn tối thiểu: {km.DieuKien.toLocaleString('vi-VN')}đ</span>
                                </>
                              )}
                            </div>
                          </div>
                ...
                <div className="text-center py-6">
                  <p className="text-gray-600">Bạn không có khuyến mãi nào khả dụng</p>
                  <button 
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 h-fit sticky top-4">
            ...
            <div className="space-y-3 mb-6">
              ...
              {discountResult.discountAmount > 0 && (
                <div className="flex justify-between text-green-600">
                  ...
                  <span>-{discountResult.discountAmount.toLocaleString('vi-VN')}đ</span>
                </div>
              )}
              ...
              <div className="flex justify-between text-lg font-bold text-gray-900">
                <span>Tổng cộng</span>
                <span className="text-green-600">{discountResult.finalTotal.toLocaleString('vi-VN')}đ</span>
              </div>
            </div>
            ...
            >
              {loading ? (
                ...
                `Xác nhận đặt hàng - ${discountResult.finalTotal.toLocaleString('vi-VN')}đ`
              )}
            </button>        
