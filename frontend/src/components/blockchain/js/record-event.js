class RecordEventHandler {
    ...
    async callRecordEventAPI(formData) {
        const response = await fetch('http://localhost:3000/api/record-event', {
            method: 'POST',
            headers: {
    ...
    if (imageData && imageData.imageUrl) {
        imageHtml = `
            <div style="margin-top: 10px;">
                ...
                <img src="http://localhost:3000${imageData.imageUrl}" 
    ...
    async loadProductHistory() {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/product-events/${productId}`);
